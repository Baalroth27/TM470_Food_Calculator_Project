...
router.post("/", async (req, res) => {
  try {
    ...
    if (
      !name ||
      !standard_measurement_unit ||
      purchase_pack_price === undefined ||
      pack_quantity_in_standard_units === undefined
    ) {
      return res
        .status(400)
    ...
    if (pack_quantity_in_standard_units <= 0) {
      return res
        .status(400)
        .json({ msg: "Pack quantity must be greater than zero." });
    }

    // --- Perform the Calculation ---
    const cost_per_standard_unit =
      purchase_pack_price / pack_quantity_in_standard_units;
...
// @desc Remove multiple ingredients by IDs
router.delete("/", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `DELETE FROM ingredients WHERE id = ANY($1::int[])`,
      [sanitizedIds]
    );
    if (result.rowCount === 0) {
      return res
        .status(404)
        .json({ msg: "None of the provided ingredient IDs were found." });
    }
    res.json({
      msg: `${result.rowCount} Ingredient(s) deleted successfully.`,
